...
// SwapToken contract: Facilitates token swapping between stackToken and FlowToken
pub contract SwapToken {
    ...
    // Function to swap tokens between stackToken and FlowToken
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {

        // Borrow stackToken and FlowToken vaults from the signer's storage
        let stackTokenVault = signer.borrow<&stackToken.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow stackToken Vault from signer")

        let flowVault = signer.borrow<&FlowToken.Vault>(from: /storage/FlowVault)
            ?? panic("Could not borrow FlowToken Vault from signer")  

        // Borrow Minter capability from stackToken
        let minterRef = signer.getCapability<&stackToken.Minter>(/public/Minter).borrow()
            ?? panic("Could not borrow reference to stackToken Minter")
        ...
        // Mint new stackTokens and deposit them to the vault
        let newstackTokenVault <- minterRef.mintToken(amount: mintedAmount)
        stackTokenVault.deposit(from: <-newstackTokenVault)
