...
// stackToken contract: Implements fungible token on Flow blockchain
pub contract stackToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @stackToken.Vault
        ...
        // Destroy the vault
        destroy() {
            stackToken.totalSupply = stackToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            stackToken.totalSupply = stackToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&stackToken.Minter>(/public/Minter, target: /storage/MinterStorage)
